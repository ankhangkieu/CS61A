...
def is_weight(w):
    ...
    "*** YOUR CODE HERE ***"
    return is_tree(w) and not branches(w)
...
def balanced(m):
    ...
    for s in sides(m):
        if not balanced(end(s)):
            return False
    return total_weight(end(sides(m)[0]))*length(sides(m)[0]) == total_weight(end(sides(m)[1]))*length(sides(m)[1])
...
def make_withdraw(balance, password):
    ...
    attemp = 3
    list_pass =[]
    def withdraw(amount, attemp_pass):
        nonlocal  attemp, balance, list_pass
        if not attemp:
            return "Your account is locked. Attempts: " + str(list_pass)
        if password != attemp_pass:
            attemp -= 1
            list_pass += [attemp_pass]
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(attemp) != int:
        return attemp
    def withdrawing(amount, password):
