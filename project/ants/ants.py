...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        "*** REPLACE THIS LINE ***"
        place, bee = self.place, None
        while place != hive and bee == None:
...
def bee_killer(bees, damage):
    """Return a list of bees which are still alive after their armors have 
    been reduced by the amount of damage.
    """
    bee_list = []
    for b in bees:
        assert isinstance(b, Bee), "bees has to be a list of bees"
        b.reduce_armor(damage)
        bee_list.append(b)
    return [b for b in bee_list if b.armor > 0]
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            self.place.bees = bee_killer(self.place.bees, self.damage) 
            self.place.remove_insect(self)
        # END Problem 4A
...
class LongThrower(ThrowerAnt):
    ...
    "*** YOUR CODE HERE"
    def nearest_bee(self, hive):
        place, bee, pos = self.place, None, 0
        while place != hive and bee == None:
            if pos >= self.min_range:
                bee = random_or_none(place.bees)
            place = place.entrance
            pos += 1
        return bee
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        place, bee, pos = self.place, None, 0
        while place != hive and bee == None and pos <= self.max_range:
            bee = random_or_none(place.bees)
            place = place.entrance
            pos += 1
        return bee
...
# The WallAnt class
class WallAnt(Ant):
    """A WallAnt has a large armor value but does not damage."""
...
class ScubaThrower(ThrowerAnt):
    """A ScubaThrower is a ThrowerAnt which is watersafe."""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if not self.is_queen:
            ...
            while place != None:
                if place.ant != None:
                    if place.ant.container and place.ant.ant and (place.ant.ant not in self.powered_ants):
                        place.ant.ant.damage *= 2
                        self.powered_ants += [place.ant.ant]
                    if place.ant not in self.powered_ants:
                        place.ant.damage *= 2
                        self.powered_ants += [place.ant]
                place = place.exit
        # END Problem 9
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** REPLACE THIS LINE ***"
    normal_act = bee.action
    def applying(colony):
        nonlocal duration
        if duration:
            duration -= 1
            return effect(normal_act)(colony)
        return normal_act(colony)
    bee.action = applying
    # END Problem EC
